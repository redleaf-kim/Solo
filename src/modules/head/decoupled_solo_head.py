...
class DSoloHead(nn.Module):
    ...
    def _init_layers(self):
        ...
        self.mask_x_convs = nn.ModuleList()
        self.mask_y_convs = nn.ModuleList()
        ...
        self.solo_mask_x = nn.ModuleList([
            nn.Conv2d(self.feat_channels, num, 1, padding=0) for num in self.grid_num
        ])
        self.solo_mask_y = nn.ModuleList([
            nn.Conv2d(self.feat_channels, num, 1, padding=0) for num in self.grid_num
        ])


    def init_weights(self):
        for m in self.mask_x_convss:
            normal_init(m.conv, std=0.01)
        for m in self.mask_y_convss:
            normal_init(m.conv, std=0.01)
        for m in self.cate_convs:
            normal_init(m.conv, std=0.01)
        bias_maks = bias_init_with_prob(0.01)
        for m in self.solo_mask_x:
            normal_init(m, std=0.01, bias=bias_maks)
    ...
    def forward(self, feats):
        ...
        x_mask_pred, y_mask_pred, cate_pred = multi_apply(self.forward_single, new_feats,
                                                        list(range(len(self.grid_num))),
                                                        upsampled_size=upsampled_size)
        return x_mask_pred, y_mask_pred, cate_pred
    ...
    def forward_single(self, x, idx, upsampled_size=None):
        ...
        x_mask_feat = F.interpolate(x_mask_feat, scale_factor=2, mode='bilinear')
        y_mask_feat = F.interpolate(y_mask_feat, scale_factor=2, mode='bilinear')

        x_mask_pred = self.solo_mask_x[idx](x_mask_feat)
        y_mask_pred = self.solo_mask_y[idx](y_mask_feat)
        ...
        if not self.training:
            x_mask_pred = F.interpolate(x_mask_pred.sigmoid(), size=upsampled_size, mode='bilinear')
            y_mask_pred = F.interpolate(y_mask_pred.sigmoid(), size=upsampled_size, mode='bilinear')
